...
class ProfilepicScreen extends GetWidget<ProfilepicController> {
  ...
  Widget build(BuildContext context) {
    return SafeArea(child: Scaffold(appBar: _buildAppBar(),
        body: SizedBox(width: SizeUtils.width,
            child: SingleChildScrollView(padding: EdgeInsets.only(top: 4.v),
                child: Container(margin: EdgeInsets.only(bottom: 5.v),
                  ...
                  Align(alignment: Alignment.centerLeft,
                      child: Container(width: 336.h,
                          margin: EdgeInsets.only(right: 21.h),
                          child: Text("msg_upload_a_profile".tr, maxLines: 2,
                              overflow: TextOverflow.ellipsis,
                              style: theme.textTheme.bodyMedium!.copyWith(
                                  height: 1.35)))),
                  SizedBox(height: 100.v),
                  Container(margin: EdgeInsets.symmetric(horizontal: 50.h),
                      padding: EdgeInsets.symmetric(
                          horizontal: 90.h, vertical: 10.v),
                      decoration: BoxDecoration(image: DecorationImage(
                          image: /*imageFile== null  ? */fs.Svg(ImageConstant.imgGroup2) /*:  FileImage(
                              File(imageFile?.path ?? ""))*/, fit: BoxFit.cover)
                ...
                Opacity(opacity: 0.5,
                    child: CustomElevatedButton(
                        text: "msg_complete_registration".tr, onPressed: () {
                      onTapNext();
  ...
  PreferredSizeWidget _buildAppBar() {
    return CustomAppBar(leadingWidth: 26.h,
        leading: AppbarLeadingImage(imagePath: ImageConstant.imgArrowLeft,
            margin: EdgeInsets.only(left: 16.h, top: 17.v, bottom: 22.v),
            onTap: () {
              onTapArrowLeft();
            }),
        actions: [
          Container(height: 44.adaptSize,
              width: 44.adaptSize,
              margin: EdgeInsets.symmetric(horizontal: 16.h, vertical: 6.v),
              child: Stack(alignment: Alignment.bottomCenter,
                  children: [
                    Align(alignment: Alignment.center,
                        child: Container(height: 44.adaptSize,
                            width: 44.adaptSize,
                            decoration: BoxDecoration(
  ...
  onTapNext() {
    ...
    controller.apiClient.updateUser(argumentData['userId'], map)!.then((
        value) => {Get.toNamed(AppRoutes.profilepicScreen,)});
  }
  ...
  void _settingModalBottomSheet(context) {
    showModalBottomSheet(context: context, builder: (BuildContext bc) {
      return Container(child: new Wrap(children: <Widget>[
        new ListTile(title: new Text('Gallery'),
            onTap: () => {
              imageSelector(context, "gallery"),
              Navigator.pop(context),
            }),
        new ListTile(title: new Text('Camera'),
          onTap: () => {
